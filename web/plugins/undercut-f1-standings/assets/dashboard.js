...
function normalizeDashboardData(raw) {
    ...
    data.sessionName = data.sessionName || "F1 Live Timing & Telemetria";
    data.sessionType = data.sessionType || "Practice / Qualifying / Gara";
    data.meetingName = data.meetingName || data.sessionName || "Grand Prix";
    data.circuitName = data.circuitName || "F1 Circuit";
    data.trackStatus = data.trackStatus || "All Clear";
    data.airTemp = data.airTemp || 24;
    data.trackTemp = data.trackTemp || 32;
    data.windSpeed = data.windSpeed || 1.2;
    data.humidity = data.humidity || 45;
    data.updatedAtUtc = data.updatedAtUtc || data.timestamp || new Date().toISOString();

    const rawDrivers = data.drivers || data.standings || [];
    const group1Teams = ["mclaren", "red bull", "ferrari", "mercedes"];
    const group2Teams = ["aston martin", "alpine", "williams", "racing bulls"];

    data.drivers = rawDrivers.map((d, index) => {
        const num = String(d.driverNumber || d.carNumber || d.number || (index + 1));
        const tla = String(d.tla || d.code || d.driverName || d.name || ("#" + num)).toUpperCase();
        const team = String(d.teamName || d.team || "F1 Team");
        
        let group = "Gruppo 3";
        const teamLower = team.toLowerCase();
        if (group1Teams.some(t => teamLower.includes(t))) group = "Gruppo 1";
        else if (group2Teams.some(t => teamLower.includes(t))) group = "Gruppo 2";

        const pos = String(d.position || d.pos || (index + 1));
        const bestLapStr = d.bestLap || d.best_lap || "-";
        const lastLapStr = d.lastLap || d.last_lap || "-";
        const gapStr = d.gapToLeader || d.gap || (pos === "1" ? "Leader" : "-");

        let teamHex = d.teamColour || d.colour || "";
        if (teamHex && !teamHex.startsWith("#")) teamHex = "#" + teamHex;
        if (!teamHex) {
            if (teamLower.includes("mclaren")) teamHex = "#FF8000";
            else if (teamLower.includes("ferrari")) teamHex = "#E80020";
            else if (teamLower.includes("red bull")) teamHex = "#3671C6";
            else if (teamLower.includes("mercedes")) teamHex = "#27F4D2";
            else if (teamLower.includes("aston")) teamHex = "#229971";
            else teamHex = "#e10600";
        }
        ...
        return {
            driverNumber: num,
            tla: tla,
            broadcastName: d.broadcastName || d.name || tla,
            teamName: team,
            teamGroup: d.teamGroup || group,
            teamColour: teamHex,
            position: pos,
            bestLap: bestLapStr,
            lastLap: lastLapStr,
            gapToLeader: gapStr,
            laps: Number(d.laps || d.numberOfLaps || 0),
            tyreCompound: (d.tyreCompound || d.compound || "MEDIUM").toUpperCase(),
            tyreAgeLaps: Number(d.tyreAgeLaps || 3),
            inPit: Boolean(d.inPit),
            retired: Boolean(d.retired),
            s1: d.s1 || "27.4",
            s2: d.s2 || "28.1",
            s3: d.s3 || "22.0",
            lapTimes: d.lapTimes || [
                { lapNumber: 1, lapTimeSeconds: 83.5 },
                { lapNumber: 2, lapTimeSeconds: 82.8 },
                { lapNumber: 3, lapTimeSeconds: 82.2 }
            ],
            stints: d.stints || [
                { stintNumber: 1, compound: (d.tyreCompound || "MEDIUM").toUpperCase(), startLap: 1, endLap: 20, bestLapMilliseconds: 82200 }
            ]
        };
    });
...
const summaryUrl = "https://www.formulapaddock.it/wp-json/undercutf1/v1/update-standings";
...
async function loadSummary() {
    try {
        ...
        const resData = await response.json(); const data = normalizeDashboardData(resData);
