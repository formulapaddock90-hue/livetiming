...
public static partial class CommandHandler
{
    ...
    )
    {
        ...
        builder
            ...
            .AddHostedService<Api.StandingsExportService>();
        ...
        if (options.ApiEnabled)
        {
            ...
            {
                options = options with { ApiKey = GenerateApiKey() };
            }
        ...
        builder.Services.AddOptions<ConsoleOptions>()
            .Configure(opt =>
            {
                // Copy properties from loaded options
                foreach (var prop in typeof(ConsoleOptions).GetProperties())
                {
                    // Skip read-only properties and static properties
                    if (prop.SetMethod == null || prop.GetMethod == null)
                        continue;

                    var value = prop.GetValue(options);
                    prop.SetValue(opt, value);
                }
            });
        ...
        // The Swagger UI only respects the Mvc JsonOptions, so set both even though we only need the Http.Json one for minimal API's
        ...
        if (options.ApiEnabled)
        {
            ...
            app.MapInfoEndpoints().MapControlEndpoints().MapTimingEndpoints().MapDashboardEndpoints().MapExportEndpoints();

            app.Lifetime.ApplicationStarted.Register(() =>
            {
                OpenDashboardInBrowser(app.Logger, "http://localhost:61937");
                if (!string.IsNullOrEmpty(options.ApiKey))
                {
                    app.Logger.LogInformation("API Key: {ApiKey}", options.ApiKey);
                }
            });
        }
    ...
    private static void OpenDashboardInBrowser(ILogger logger, string url)
    {
        try
        {
...
#pragma warning restore RS0030
            logger.LogInformation("Opened dashboard in browser: {Url}", url);
        }
        catch (Exception ex)
        {
            logger.LogWarning(ex, "Failed to open browser automatically for {Url}", url);
        }
    }
    ...
    private static string GenerateApiKey()
    {
        ...
        rng.GetBytes(buffer);
        return "uk_" + Convert.ToBase64String(buffer).Replace("+", "-").Replace("/", "_").TrimEnd('=');
    }
}
